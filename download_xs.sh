...
# This script is copied from Cardinal and edited to use endf8.1
# https://github.com/neams-th-coe/cardinal/blob/devel/scripts/download-openmc-cross-sections.sh
set -e

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
DOWNLOAD_DIR=${1:-$(realpath $SCRIPT_DIR/../..)/cross_sections}
XS_VERSION=endfb-viii.1-hdf5
XS_DIR=${DOWNLOAD_DIR}/${XS_VERSION}

if [[ ! -d ${XS_DIR} ]]; then
  mkdir -p ${XS_DIR}
  echo "Downloading cross sections to ${XS_DIR}"
  XS_URL=https://anl.box.com/shared/static/6qr7jezzihkj9p9esl5jn19qgpujyjyz.xz
