...
def generate_model(cd_angle, T_fuel, T_prop, rho_prop):
    ...
    hex_lattice.outer = unfueled_universe
    hex_lattice.center = (0, 0)
    hex_lattice.pitch = [f2f]

    # This csv is formulated like: fuel_id, x_pos, y_pos for all fuel elements.
    with open("xe_elements_pos.csv", "r") as file:
        ...
        for line in lines:
            ...
            # Find the lattice index for the given x, y
            lat_elem_index = hex_lattice.find_element((x, y, 0))[0]

            # Now, find the universe index from the lattice index
            hex_i, hex_j = hex_lattice.get_universe_index(lat_elem_index)

            # xx-xA means unfueled element, the centermost element is 00-00 so that
            # must be accounted for too.
            if key.endswith("A") or key.endswith("0"):
                universes[hex_i][hex_j] = ue_universe

            else:
                loading_code = get_loading_code(key)
                fe_universe = fe_universes[loading_code]

                universes[hex_i][hex_j] = fe_universes[loading_code]
